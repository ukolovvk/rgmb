...
public class ImpBookDAO implements BookDAO {
    ...
    private final String generalSql = "WITH books_genres_table AS (SELECT books.book_id, string_agg(genre.genre_name,',') as genres\n" +
            ...
            "GROUP BY (books.book_id)\n" +
            ")\n" +
            "SELECT books.book_id, books.title, bgt.genres, bat.authors, books.rating, books.page_count, books.year, books.annotation \n" +
            "FROM books\n" +
    ...
    public int add(Book book) {
        String SQL = "INSERT INTO books(title,page_count,rating,year, annotation) VALUES(?,?,?,?,?) RETURNING book_id";
        int bookID = template.queryForObject(SQL,new BookRowMapperForFindByBookName(), book.getTitle(),book.getSize(),book.getRating(),book.getYear(),book.getAnnotation());
